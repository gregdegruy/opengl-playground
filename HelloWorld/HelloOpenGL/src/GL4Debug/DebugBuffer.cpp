...
#define GLCall(x) helloDebugger.GLClearError();\
	x;\
	ASSERT(helloDebugger.GlLogCall(#x, __FILE__, __LINE__))
...
int DebugBuffer::run()
{
	...
	DebugShader optimized_shader;
	DebugShaderProgramSource source = optimized_shader
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		GLCall(glDrawElements(GL_TRIANGLES, num_indicies, GL_INT, nullptr)); // nullptr since we bind buffers using glGenBuffers
...
void DebugBuffer::createBuffer(unsigned int& buffer, unsigned int& index_buffer)
{
...
void DebugBuffer::bindVertex(unsigned int index_to_bind, unsigned int num_floats_per_vertex) {
